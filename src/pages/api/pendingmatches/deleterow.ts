...
export default async function DeletePendingMatchRow(req: NextApiRequest, res: NextApiResponse) {
  // we will be responding with JSON in this file, declare this.
  res.setHeader('Content-Type', 'application/json')

  const sql = `DELETE FROM pendingmatches WHERE mentee_id=$1 AND mentor_id=$2;`
  const values = [req.body.mentee_id, req.body.mentor_id]
