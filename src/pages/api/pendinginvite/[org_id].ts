...
export default async function PendingInvites(req: NextApiRequest, res: NextApiResponse) {
  // we will be responding with JSON in this file, declare this.
  res.setHeader('Content-Type', 'application/json')

  const sql = `SELECT * FROM pendinginvite WHERE org_id=$1;`
  const values = [req.query.org_id]
